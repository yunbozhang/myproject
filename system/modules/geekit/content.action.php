...
HTML;
	...
	public function goods_edit(){
		...
		if(isset($_POST['dosubmit'])){		
			...
			$sql="UPDATE `@#_shoplist` SET `cateid` = '$cateid',
										   ...
										   `xsjx_time` = '$xsjx_time',
										    `zhiding`='".htmlspecialchars($_POST['zhiding'])."',
											`renqipos` = '$renqipos'  ,
											`newpos` = '$newpos' ,		
											`posthumb` = '$posthumb' ,	
										   `pos` = '$goods_key_pos'
	...
	public function goods_add(){		
		...
		if(isset($_POST['dosubmit'])){		
			...
			$query_1 = $this->db->Query("INSERT INTO `@#_shoplist` (`zhiding`,`cateid`, `brandid`, `title`, `title_style`, `title2`, `keywords`, `description`, `money`, `yunjiage`, `zongrenshu`, `canyurenshu`,`shenyurenshu`, `qishu`,`maxqishu`,`thumb`, `picarr`, `content`,`xsjx_time`,`renqi`,`pos`,`bannershop`,`renqipos`,`newpos`,`posthumb`, `time`) VALUES ('".htmlspecialchars($_POST['zhiding'])."','$cateid', '$brandid', '$title', '$title_style', '$title2', '$keywords', '$description', '$money', '$yunjiage', '$zongrenshu', '$canyurenshu','$shenyurenshu', '1','$maxqishu', '$thumb', '$picarr', '$content','$xsjx_time','$goods_key_renqi', '$goods_key_pos','$goods_key_bannershop','$renqipos','$newpos','$posthumb','$time')");			
